...
export const getStaticPaths = async () => {
  const res = await client.getEntries({
    content_type: "recipe", // Content type ID in contentful
  });
  ...
  return {
    paths: paths,
    fallback: true, // If false, new pages will return 404 page.
  };
};
...
export async function getStaticProps({ params }) {
  const { items } = await client.getEntries({
    content_type: "recipe",
    "fields.slug": params.slug, // Retrieve only particular item
  });
